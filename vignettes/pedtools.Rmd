...
In most cases it is not recommended for regular users to interact directly with the internal slots of a `ped`, since this can have unfortunate consequences unless you know exactly what you are doing. Instead, one should use accessor functions like `relabel()`, `parents()` and `getSex()`. The most important accessors are described within this vignette, while others are documented in the help page `?ped_utils`.
...
Several options are available for controlling the appearance of individual pedigree members and the plot as a whole. A complete overview can be found in the documentation `?plot.ped`, but a quick example should get you started:
...
### Example 3: A family tree
For our final example we consider the family tree of a single person going back 3 generations:

```{r echo = FALSE, fig.width = 2.5, fig.width = 4}
...
We _could_ do this by starting with the bottom trio, followed by a series of calls to `addParents()`. Instead we will use it as a showcase for the `mergePed()` function. This powerful function is based on a simple idea: Given two pedigrees, "glue together" members with matching ID labels, and check that the result is a valid pedigree. 

When creating the parts that are to be merged, it is usually best to keep the overlap minimal. In our code below we chose the mother to be the single point of contact. To understand the relabelling, plot `y1` and `y2` after each command.
